...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'role:admin'])->group(function () {
    ...
    // bentrok dengan parameter {pembayaran}
    Route::get('pembayaran/pelanggan/{pelanggan}/tagihan', [AdminPembayaran::class, 'tagihanPelanggan'])
         ->name('pembayaran.pelanggan.tagihan');
    Route::post('pembayaran/tunai',    [AdminPembayaran::class, 'bayarTunai'])->name('pembayaran.tunai');
    Route::post('pembayaran/pakasir',  [AdminPembayaran::class, 'bayarPakasir'])->name('pembayaran.pakasir');
    ...
    // Cek status transaksi Pakasir (admin polling AJAX)
    Route::get('pakasir/status/{ref}', [PakasirController::class, 'cekStatus'])->name('pakasir.status');
...
Route::prefix('pelanggan')->name('pelanggan.')->middleware(['auth', 'role:pelanggan'])->group(function () {
    ...
    // ── Pakasir self-service ────────────────────────────────────────
    Route::post('/tagihan/{tagihan}/pakasir', [PakasirController::class, 'bayar'])
         ->name('pakasir.bayar');

    Route::get('/pakasir/status/{ref}', [PakasirController::class, 'cekStatus'])
         ->name('pakasir.cek-status');

    Route::get('/pakasir/sukses', [PakasirController::class, 'sukses'])
         ->name('pakasir.sukses');

    Route::get('/pakasir/gagal',  [PakasirController::class, 'gagal'])
         ->name('pakasir.gagal');
    // ── End Pakasir ─────────────────────────────────────────────────
