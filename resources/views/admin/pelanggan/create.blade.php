      ...
      <div>
        ...
        <div>
          ...
          <select name="jorong_id" class="sel-wilayah w-full px-3 py-2.5 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500">
            ...
            <option value="{{ $j->id }}" {{ old('jorong_id') == $j->id ? 'selected' : '' }}>{{ $j->nama_jorong }}</option>
            @endforeach
          </select>
        </div>
...
<script>
const defLat   = {{ old('latitude',  isset($pelanggan) ? ($pelanggan->latitude  ?? -0.1277) : -0.1277) }};
const defLng   = {{ old('longitude', isset($pelanggan) ? ($pelanggan->longitude ?? 100.1746) : 100.1746) }};
