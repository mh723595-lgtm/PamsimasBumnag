    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                @php $grouped = $jorong->groupBy('kabupaten'); $no = 1; @endphp
                        ...
                        <td colspan="7" class="px-4 py-2">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span class="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">
                                    ...
                                    <span class="font-normal text-gray-400 normal-case tracking-normal">— {{ $items->first()->provinsi }}</span>
                                    @endif
                                </span>
                                <button onclick="bukaModalTambahDiWilayah('{{ addslashes($items->first()->provinsi) }}','{{ addslashes($kabupaten) }}','{{ addslashes($items->first()->kecamatan) }}','{{ addslashes($items->first()->desa) }}','{{ addslashes($items->first()->nagari) }}')"
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        <td class="px-4 py-3 font-medium text-gray-800 dark:text-white">
                            <div class="flex items-center gap-2">
                                @if($kabupaten)<span class="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0"></span>@endif
                                {{ $j->nama_jorong }}
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg text-xs font-mono">{{ $j->kode_jorong ?? '-' }}</span>
                        </td>
                        <td class="px-4 py-3 text-xs text-gray-500">
                            @if($j->kecamatan || $j->desa)
                                @if($j->kecamatan)<span class="text-gray-600 dark:text-gray-400">Kec. {{ $j->kecamatan }}</span>@endif
                                @if($j->desa)<span class="text-gray-400"> · {{ $j->desa }}</span>@endif
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="editJorong({{ $j->id }},'{{ addslashes($j->nama_jorong) }}','{{ addslashes($j->kode_jorong) }}','{{ addslashes($j->provinsi) }}','{{ addslashes($j->kabupaten) }}','{{ addslashes($j->kecamatan) }}','{{ addslashes($j->desa) }}','{{ addslashes($j->nagari) }}','{{ addslashes($j->keterangan) }}')"
...
{{-- ═══════════════════════════════════════════
    Modal Tambah Jorong
════════════════════════════════════════════ --}}
<div id="modal-tambah" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div class="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Desa / Kelurahan</label>
                ...
                <div class="mt-3">
                    <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Nagari <span class="font-normal text-gray-400">(opsional)</span></label>
            ...
            <div>
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                    <span class="w-5 h-5 rounded-full bg-emerald-500 text-white text-xs flex items-center justify-center font-bold">2</span>
        ...
        {{-- Footer --}}
        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex gap-3 justify-end flex-shrink-0 bg-gray-50 dark:bg-gray-800/50">
            <button onclick="tutupModalTambah()" class="px-4 py-2 rounded-xl text-sm text-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">Batal</button>
...
{{-- ═══ Modal Edit ═══ --}}
<div id="modal-edit" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex gap-3 justify-end flex-shrink-0">
            <button onclick="document.getElementById('modal-edit').classList.add('hidden')" class="px-4 py-2 rounded-xl text-sm text-gray-600 hover:bg-gray-100 transition-colors">Batal</button>
            <button onclick="simpanEdit()" class="px-4 py-2 rounded-xl text-sm font-semibold bg-blue-500 hover:bg-blue-600 text-white transition-colors">Simpan Perubahan</button>
        </div>
    </div>
</div>
...
// ── Modal Edit ─────────────────────────────────────────────
function editJorong(id, nama, kode, provinsi, kabupaten, kecamatan, desa, nagari, keterangan) {
